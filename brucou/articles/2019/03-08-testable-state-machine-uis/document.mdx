...
export const demoboardHelpers = {
  'demo1/App.js': require('!raw-loader!./demo1/App.js'),
  'demo1/helpers.js': require('!raw-loader!./demo1/helpers.js'),
  'demo1/index.css': require('!raw-loader!./demo1/index.css'),
  'demo1/index.js': require('!raw-loader!./demo1/index.js'),
  'demo1/properties.js': require('!raw-loader!./demo1/properties.js'),
  'demo1/uikit.css': require('!raw-loader!./demo1/uikit.css'),
}

So, you just finished a project. Months of crazy deadlines, 
constant changes in specifications, and an endless stream of issues later, *you did it!* Happy as 
Ulysses on his way back to Ithaka, you enjoy a few days at home and start binge-watching movies. 
For some reason, unhappy with the time it takes you to find movies you like, you decide -- you 
confident programmer -- that you will make an online interface to the [The Movie Database (TMDb)](https://www.themoviedb.org/?language=en-US).

It should be easy, right? A query field, a few network requests, a way to display results, what can possibly go 
wrong? So you take the informal specifications in your head, and imagine how you would like to use the app. You craft detailed specifications, user flows, design the screens, and do the programming. It looks great, and thanks to your state charts, it's bug free from the get go!

---

In this guide, we are going to use the example of a simple online movie database to explore how state machines can help you to create **robust**, **maintainable** and **testable** user interfaces. We'll start by describing the application's requirements, and end by presenting the state machine formalism. In the process, we'll find an implicit state machine, and see the advantages of using an explicit one instead.

While this article covers the subject relatively fast, I 
encourage you to pause and rewind when lost, consult the code examples, and post your questions on the [dedicated SO tag](https://stackoverflow.com/questions/tagged/state-transducer) or come and discuss the topic with likeminded readers at the [statecharts group](https://spectrum.chat/statecharts).
...
In order to scope and guide the implementation, you start by writing the detailed specifications of the 
**interface behaviour** in a more formalized form, taking a chapter from BDD. You come up with 11 assertions, each which might visually look something like this:

<Doc.Details aside>

Realistically, this is closer to taking a *page* from BDD than a *chapter*. In true BDD, you may consolidate these 11 assertions into three scenarios. Let's not bother, shall we.

</Doc.Details>
</Doc.AsideTop>

<Specs specs={[
[require('./app screenshot init - pending.png'),
...
`GIVEN user navigated to [url]
  AND query field has not changed 
WHEN default query is successful
THEN display result screen`],
[require('./app screenshot init - error.png'),
`GIVEN url not [url]
  AND user navigates to [url]
  AND query field has not changed 
WHEN default query is not successful
THEN display error screen`],
[require('./app screenshot query - pending.png'),
`GIVEN user navigated to [url]
  AND query field has not changed 
WHEN query field changes
AND query field is not empty
THEN 
- query for movies containing the content of <query> field
- display loading screen`],
[require('./app screenshot query - success.png'),
`GIVEN user navigated to [url]
  AND query field changed
  AND query field is not empty
WHEN query is successful
THEN display result screen`],
[require('./app screenshot query - error.png'),
`GIVEN user navigated to [url]
  AND query field changed
  AND query field is not empty
WHEN query is not successful
THEN display error screen. `],
[require('./app screenshot init - pending.png'),
`GIVEN user navigated to [url]
  AND query field changed
WHEN query field changes
AND query field is empty
THEN 
- display loading screen
- query for movies in some default way`],
[require('./app screenshot query detail - pending.png'),
`GIVEN user navigated to [url]
  AND query field changed
  AND query field is not empty
...
[require('./app screenshot query detail - success.png'),
`GIVEN user navigated to [url]
  AND query field changed
  AND query field is not empty
  AND query was successful
  AND user clicked on a movie  
WHEN movie detail query is successful
THEN display movie detail screen`],
[require('./app screenshot query detail - pending.png'),
`GIVEN user navigated to [url]
  AND query field changed
  AND query field is not empty
  AND query was successful
  AND user clicked on a movie  
WHEN movie detail query is not successful
THEN display movie detail error screen`],
[require('./app screenshot query - success.png'),
`GIVEN user navigated to [url]
  AND query field changed
  AND query field is not empty
  AND query was successful
  AND user clicks on a movie
  AND movie detail query is successful
WHEN user clicks outside of the movie detail
THEN display result screen corresponding to the query`],
]} />

### Implementation

A basic implementation of the above specifications can be found in these four GitHub repositories:

- [Spec 1](https://github.com/brucou/movie-search-app/tree/specs-S1)
- [Specs 2-3](https://github.com/brucou/movie-search-app/tree/specs-S2)
- [Specs 4-7](https://github.com/brucou/movie-search-app/tree/specs-S4)
- [All Specs](https://github.com/brucou/movie-search-app/tree/specs-all)

I've also placed it within this demoboard, for your convenience:

*If you'd like more space to work with the files, try clicking the fullscreen button at the top right of the editor!*

```js
//--- App.js <-- demo1/App.js
//--- helpers.js <-- demo1/helpers.js
//--- properties.js <-- demo1/properties.js
//--- index.js <-- demo1/index.js
//--- index.css <-- demo1/index.css
//--- uikit.css <-- demo1/uikit.css
```

This implementation uses `React` exclusively as a DOM library, and uses hyperscript helpers, so the screens follow a html-like form. It also uses a standard model-view-controller division:
