...
-   Matcher functions like `mount()`, `lazy()` and `route()` are just Generator Functions. In fact, it's entirely possible to create [custom matchers](https://frontarm.com/navi/en/reference/matchers/#custom-matchers). For instance, you could create a `withTimeout()` matcher that switches routes based on how long they take to load. And if you *do* create useful custom matchers, send a tweet or DM to [@james_k_nelson](https://twitter.com/i/notifications) so I can spread the word 🤩
