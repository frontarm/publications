...
With a completely new API comes completely new possibilities. And to cut to the chase, Navi's new `<Router />` component uses Hooks and Suspense and its gonna blow your mind 🤯. It lets you do things like visualize transitions between pages with an animated loading bar... in 3 lines of code. And the `navi-scripts` tool lets you statically render create-react-app based projects, *without ejecting from create-react-app*.

Amazing, right? But before we break out the hooks, *what the hell is a Navi `<Router />`?*
...
It could route **over 9000** routes. In fact, it could route even more, because it lets you dynamically `import()` entire routing trees on demand. But how?

The trick is in how you declare routes. It's simple -- you get a bunch of functions like `lazy()`, `mount()` and `route()`, and you just mash them together to map URLs to (possibly asynchronous) content.
...
Ok, so you've got a `<Router>` with a couple routes, including a shop's landing page and a lazily loadable `/products` URL. Next step: you need to decide where to render the current route's `view` element. And to do that, you just plonk down a `<View />` somewhere inside your `<Router>`.
...
Ok, so you've seen how to render a route's view. But your route also defined a `getData()` function.
...
There's just one problem. Doing this with currently available tools is ridiculously hard, right? Well actually... You can add it to the above demo in just 3 lines of code, using the `useLoadingRoute()` hook and the `react-loading-indicator` package.
...
Go ahead and try clicking between these pages a few times. Did you notice how smooth the transition is? No? It was so smooth that you didn't notice anything particularly special? Great! *That's exactly the experience that your users want.*
...
Ok, so even if you're not Obi-Wan Kenobi, Navi needs *you*. Every little bit of help... helps.

Actually, a lot of little things are way more helpful than they first appear. Can you try Navi out and file an issue for any missing features? *Awesome.* Can you make tiny improvements to the docs as you learn? *Radical.* Can you put something small online and add it to the [README](https://github.com/frontarm/navi#whos-using-navi)'s list of sites using Navi? *Phenomenal.*
...
-   If you'd like to see this project grow, please give it a [🌟 on Github](https://github.com/frontarm/navi)!
